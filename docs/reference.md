...
- **Agent 产物**：已补 `alphasift overview --json/--explain`、`alphasift report <run_id>` 和 `alphasift serve` 只读本地 JSON API，可把策略分组、策略筛选 facets、策略准备度、数据源健康、最近运行、next actions 和单次选股运行输出为稳定 payload，便于被通知助手、Web UI 或 MCP/HTTP 服务消费；下一步补报告模板和更完整的 UI 审批流。
